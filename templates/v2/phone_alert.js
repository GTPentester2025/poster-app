// v2 template — phone-alert (style: bullet). Reinterpretation of the AB InBev
// "Alerta de phishing" poster (source: 1.html) at the v2 canvas scale. The
// archetype is a phone mockup showing three stacked instruction cards; here
// reinterpreted as: a yellow-accented headline band at the top, a large
// phone-frame decor region (rounded rect body + notch bar drawn with rect and
// circle), three instruction cards rendered as a stacked message list inside
// the phone frame, and a bold CTA footer. The dark-system landscape variant
// moves the phone frame to the right column and places the headline + decor
// on the left. Yellow accent = palette.primary throughout.
//
// Source → port: h1 "Alerta de phishing" → headline zone; three .box items →
// one message Textbox per block inside the phone frame; .footer → ctaBar;
// phone chrome (.phone / .notch / .speaker / .camera) → rect + circle decor.
...
export default {
  ...
  name: 'Phone Alert',
  style: 'bullet',
  description: 'A dark-canvas phishing alert poster built around a phone-frame motif: a yellow headline band at the top, a styled phone mockup holding three stacked instruction cards (one per step), and a bold yellow CTA footer.',
  contentSchema: {
